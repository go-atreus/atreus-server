...
// getUserInfo ...
func (m *UserDefaultServer) getUserInfo(ctx context.Context, in *UserInfoReq) (*SysUser, error) {
	out := &SysUser{}
	return out, nil
}

// CreateSysUser ...
func (m *UserDefaultServer) CreateSysUser(ctx context.Context, in *SysUser) (*SysUser, error) {
	db := m.DB
	if custom, ok := interface{}(in).(UserSysUserWithBeforeCreateSysUser); ok {
		var err error
		if db, err = custom.BeforeCreateSysUser(ctx, db); err != nil {
			return nil, err
		}
	}
	res, err := DefaultCreateSysUser(ctx, in, db)
	if err != nil {
		return nil, err
	}
	out := res
	if custom, ok := interface{}(in).(UserSysUserWithAfterCreateSysUser); ok {
		var err error
		if err = custom.AfterCreateSysUser(ctx, out, db); err != nil {
			return nil, err
		}
	}
	return out, nil
...
// UserSysUserWithBeforeCreateSysUser called before DefaultCreateSysUserSysUser in the default CreateSysUser handler
type UserSysUserWithBeforeCreateSysUser interface {
	BeforeCreateSysUser(context.Context, *gorm.DB) (*gorm.DB, error)
}

// UserSysUserWithAfterCreateSysUser called before DefaultCreateSysUserSysUser in the default CreateSysUser handler
type UserSysUserWithAfterCreateSysUser interface {
	AfterCreateSysUser(context.Context, *SysUser, *gorm.DB) error
}
